    ...
    private async getAllUsers(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            const users = await this.userService.find({}, {
                select: ['email', 'firstName', 'lastName', 'isVerified', 'isCompleted', 'isKYCDone', 'isKYCRejected', 'kycRejectionReason', 'role', 'status', 'phoneNumber', 'whatsappNumber', 'countryOfOrigin', 'countryOfResidence', 'purpose', 'typeOfBusiness', 'monthlyVolume', 'hearAboutUs', 'hearAboutUsOther', 'createdAt']
            });
